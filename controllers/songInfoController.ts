...
import SongInfoService from "../services/songInfoService";
...
class SongInfoController {
  static async getList(ctx: Koa.Context) {
    ...
    const { count, rows } = await SongInfoService.getList(params);
    ctx.body = {
  ...
  static async getInfo(ctx: Koa.Context): Promise<any> {
    ...
    const userInfo = await SongInfoService.getInfo(id);
    if (!id) {
      ctx.body = {
